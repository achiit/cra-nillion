...
export default function Main() {
  ...
  return (
    <Container>
      <h1>MINE GAME</h1>
      <p>
        COMPLETE THE TRANSACTION AND THEN CLICK ON THE BUTTON TO PLAY THE GAME
      </p>
      <ConnectionSection
        client={client}
        userkey={userkey}
      ...
      {client && (
        <Box py={4} mb={8}>
          <h1>Perform a Transaction using Nillion</h1>
          <Tabs
            value={tabIndex}
            onChange={handleTabChange}
            aria-label="Nillion secure payment"
          >
            <Tab label="Store Secretly" />
          ...
          <Box hidden={tabIndex !== 0} py={3}>
            <Typography component="div">``
              <h2>Store Your Details and Make Payment</h2>
              <StoreSecretForm
                secretName={''}
